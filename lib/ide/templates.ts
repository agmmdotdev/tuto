...
Try both Server Action controls on \`GET /\`, including the form with JavaScript disabled, follow the in-preview links, then \`GET /control-flow?mode=missing\`, \`GET /control-flow?mode=redirect\`, \`GET /control-flow?mode=error\`, \`GET /lessons/rsc?mode=practice\`, \`GET /api/lessons/rsc?mode=practice\`, \`POST /api/lessons/rsc\` with a JSON body, \`GET /proxy-rewrite\`, \`GET /proxy-redirect\`, \`GET /proxy-response\`, \`GET /api/stream\`, \`GET /cache\`, \`GET /cache-components\`, and \`GET /fetch-cache\`. Cache semantics are independent of the host storage tier. Local lessons use the workspace-scoped memory adapter; production can install the R2/S3 value adapter plus a transactional invalidation coordinator without changing student code. Cache Component keys include the immutable generation, matching Next's build-ID safety rule.`,
      },
      {
        path: "package.json",
        language: "json",
