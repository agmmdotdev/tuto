...
const virtualWorkspaceRoot = "/__tuto_virtual__";
...
function fromVirtualWorkspacePath(filePath) {
  ...
  if (normalized === virtualWorkspaceRoot) {
    return "";
  }

  if (!normalized.startsWith(`${virtualWorkspaceRoot}/`)) {
    return null;
  }

  return normalized.slice(virtualWorkspaceRoot.length + 1);
}
...
function createTailwindScanInputs(files) {
  return [...files.entries()]
    .map(([filePath, content]) => {
      ...
      if (!tailwindSourceExtensions.has(extension) || !content) {
        return null;
      }

      return {
        file: toVirtualWorkspacePath(filePath),
        content,
        extension,
      };
    })
...
async function resolveExistingDiskCssPath(candidatePath) {
  const candidates = [candidatePath];

  if (!nodePath.extname(candidatePath)) {
    candidates.push(`${candidatePath}.css`, nodePath.join(candidatePath, "index.css"));
  }
...
function resolveTailwindPackageStylesheet(id) {
  ...
  const entryName = match[1] ?? "index";
  return require.resolve(`tailwindcss/${entryName}.css`);
}

async function resolveDiskStylesheetPath(id, base) {
  const tailwindPackagePath = resolveTailwindPackageStylesheet(id);
  ...
  if (id.startsWith(".") || id.startsWith("/")) {
    if (!base || fromVirtualWorkspacePath(base) !== null) {
      return null;
    }

    const candidatePath = id.startsWith("/")
      ? id
      : nodePath.resolve(base, id);

    return resolveExistingDiskCssPath(candidatePath);
  }

  try {
    const searchPaths = [absoluteWorkingDirectory];

    if (base && fromVirtualWorkspacePath(base) === null) {
      searchPaths.unshift(base);
    }

    return require.resolve(id, { paths: searchPaths });
  } catch {
    return null;
  }
}
...
async function loadTailwindStylesheet(files, id, base) {
  const workspaceFilePath =
    id.startsWith(".") || id.startsWith("/")
      ? (() => {
          if (id.startsWith("/")) {
            return findWorkspaceFile(files, id);
          }
          ...
          return findWorkspaceFile(files, path.normalize(path.join(workspaceBasePath, id)));
        })()
  ...
  const diskPath = await resolveDiskStylesheetPath(id, base);

  if (!diskPath) {
    throw new Error(`Could not resolve Tailwind stylesheet "${id}" from "${base}".`);
  }
...
async function loadTailwindModule(id, base, resourceHint) {
  if (id.startsWith(".") || id.startsWith("/")) {
    throw new Error(
      `Tailwind ${resourceHint} modules must come from installed packages in the stateless preview. Relative modules are not supported.`,
    );
  }
  ...
  const resolvedPath = require.resolve(id, { paths: searchPaths });
...
async function compileTailwindCss(files, filePath, contents) {
  if (!looksLikeTailwindCss(contents)) {
    return contents;
  }
  ...
  if (
    ...
    candidates = scanner.scanFiles(createTailwindScanInputs(files));
  }
