    ...
    if (layoutPath) {
      ...
      if (!context.minimumDirectory) {
        for (const slot of artifact.router.parallelRoutes ?? []) {
          ...
          const interception =
            context.matched.interception?.definition.slotDirectory ===
            slot.slotDirectory
              ? context.matched.interception
              : null;
          const slotMatch = interception
            ...
            : matchDefinitions(slot.routes, context.matched.matchUrl);
          const slotRoute = slotMatch?.route ?? slot.default;
          if (!slotRoute) {
            missingSlot = true;
          ...
          const slotResult = createBranchModel(
            ...
            modules,
            slotRoute,
            slotMatch?.params ?? context.matched.params,
            interception ? context.requestUrl : context.matched.matchUrl,
            {
              ...context,
...
function createRouteModel(
  ...
  const result = createBranchModel(
    ...
    matched.params,
    matched.matchUrl,
    { matched, minimumDirectory: undefined, requestUrl },
  );
...
function throwRenderErrors(renderErrors) {
...
function streamModel(artifact, model, finalResult) {
  const { renderErrors, stream } = modelReadableStream(artifact, model);
    ...
    runWithModuleRuntime(modules, () =>
      runWithNextCache(
        ...
        async () => {
          ...
          const streamed = streamModel(artifact, route.model, () => ({
            cacheMetrics: metrics,
          }));
          ready = true;
          resolveReady({
