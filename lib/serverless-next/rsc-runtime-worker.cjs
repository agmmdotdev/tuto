...
function createRouteModel(artifact, modules, requestUrl) {
...
async function renderRoute(artifact, url) {
  ...
  const route = createRouteModel(artifact, modules, requestUrl);
  return {
...
async function invokeAction(artifact, message) {
  ...
  try {
    ...
    const args = await rsc.decodeReply(actionBody(message), serverModuleMap);
