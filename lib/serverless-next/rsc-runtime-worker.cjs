...
function createNotFoundModel(artifact, modules, matched) {
  const route = matched?.route;
  const modulePath = route?.notFound ?? artifact.router.rootNotFound;
        ...
        runWithModuleRuntime(modules, () =>
          runWithNextCache(
            ...
            () =>
              renderModel(
                artifact,
                createNotFoundModel(artifact, modules, matched),
              ),
            request,
          ),
        ),
