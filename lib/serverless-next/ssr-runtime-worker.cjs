...
async function renderHtml(artifact, bodyBase64, formState) {
  ...
  try {
    ...
    return new Response(
      await reactDomServer.renderToReadableStream(model, { formState }),
    ).text();
  } finally {
...
async function handleMessage(message) {
  ...
  if (message.type === "render") {
    ...
    return {
      html: await renderHtml(artifact, message.bodyBase64, message.formState),
    };
  }
