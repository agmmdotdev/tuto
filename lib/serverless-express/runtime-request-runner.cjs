...
function moduleTypeForPath(filePath) {
  const loader = loaderForPath(filePath);

  return loader === "file" ? "asset" : loader;
}
...
function normalizeBuildError(error) {
  if (Array.isArray(error?.errors) && error.errors.length > 0) {
    return error.errors.map((entry) =>
      createDiagnostic("error", entry.text || entry.message || "Stateless Express build failed.", {
        filePath: entry.location?.file || entry.id
          ? normalizeWorkspacePath(entry.location?.file ?? entry.id)
          : undefined,
        line: entry.location?.line ?? entry.loc?.line,
        column:
          entry.location?.column !== undefined
...
function createRolldownWorkspacePlugin(files) {
  return {
    name: "tuto-serverless-express-workspace",
    resolveId(source, importer, options) {
      if (options?.isEntry) {
        const entryMatch = findWorkspaceFile(files, source);
      ...
      const workspaceMatch = resolveWorkspaceImport(files, source, importer);
      ...
      if (
        ...
        if (externalBareImports.has(source)) {
          return {
            id: source,
            external: true,
          };
    ...
    load(id) {
      const contents = files.get(normalizeWorkspacePath(id));
      ...
      return {
        code: contents,
        moduleType: moduleTypeForPath(id),
      };
    },
  };
}
...
function loadBundledModule(bundleText) {
  const virtualFilename = nodePath.join(
...
function normalizeCompilerKind(compiler) {
  if (compiler === "rolldown" || compiler === "sucrase") {
    return compiler;
  }
...
function toVirtualFilename(workspacePath) {
  return nodePath.join(
    absoluteWorkingDirectory,
    ...normalizeWorkspacePath(workspacePath).split("/"),
  );
}
...
function warningToDiagnostic(warning) {
  return createDiagnostic("warn", warning.text || warning.message || "Stateless Express warning.", {
    filePath: warning.location?.file || warning.id
      ? normalizeWorkspacePath(warning.location?.file ?? warning.id)
      : undefined,
    line: warning.location?.line ?? warning.loc?.line,
    column:
      warning.location?.column !== undefined
        ? warning.location.column + 1
        : warning.loc?.column,
  });
}

async function compileWorkspaceWithEsbuild(files, entryPath) {
  ...
  return {
    ...
    diagnostics: buildResult.warnings.map(warningToDiagnostic),
    loadedModule: loadBundledModule(outputFile.text),
  };
}

async function compileWorkspaceWithRolldown(files, entryPath) {
  const startedAt = Date.now();
  const { build: rolldownBuild } = await import("rolldown");
  const warnings = [];
  const buildResult = await rolldownBuild({
    ...
    platform: "node",
    plugins: [createRolldownWorkspacePlugin(files)],
    write: false,
    output: {
      exports: "auto",
      format: "cjs",
      minify: false,
      sourcemap: false,
      file: "__serverless_express_bundle__.cjs",
    },
  });
  const outputChunk = buildResult.output.find((file) => file.type === "chunk");
  ...
  return {
    compileMs: Date.now() - startedAt,
    diagnostics: warnings.map(warningToDiagnostic),
    loadedModule: loadBundledModule(outputChunk.code),
  };
}

function getSucraseTransforms(filePath) {
  const extension = path.extname(filePath).toLowerCase();
  ...
  if (extension === ".jsx" || extension === ".tsx") {
    transforms.unshift("jsx");
  }

  return transforms;
}

function loadWorkspaceModuleWithSucrase(
  files,
  workspacePath,
  moduleCache,
  parentModule = module,
  ...
  if (typeof contents !== "string") {
    throw new Error(`Missing workspace module: ${normalizedPath}`);
  }
  ...
  if (extension === ".json") {
    try {
      const jsonModule = { exports: JSON.parse(contents) };
      moduleCache.set(normalizedPath, jsonModule);
      return jsonModule.exports;
    } catch (error) {
      throw new Error(
        error instanceof Error
          ? `Unable to parse JSON workspace module ${normalizedPath}: ${error.message}`
          : `Unable to parse JSON workspace module ${normalizedPath}.`,
      );
    }
  }

  if (extension === ".txt" || extension === ".md") {
    const textModule = { exports: contents };
    moduleCache.set(normalizedPath, textModule);
    return textModule.exports;
  }

  if (![".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"].includes(extension)) {
    const assetModule = { exports: normalizedPath };
    moduleCache.set(normalizedPath, assetModule);
    return assetModule.exports;
  }

  const virtualFilename = toVirtualFilename(normalizedPath);
  const runtimeModule = new Module(virtualFilename, parentModule);
  const nodeRequire = createRequire(virtualFilename);
  const localRequire = (request) => {
    if (request.startsWith(".") || request.startsWith("/")) {
      const workspaceMatch = resolveWorkspaceImport(files, request, normalizedPath);

      if (!workspaceMatch) {
        throw new Error(
          `Unable to resolve workspace import "${request}" from ${normalizedPath}.`,
        );
      }

      return loadWorkspaceModuleWithSucrase(
        files,
        workspaceMatch,
        moduleCache,
        runtimeModule,
  ...
  localRequire.resolve = (request) => {
    if (request.startsWith(".") || request.startsWith("/")) {
      ...
      return toVirtualFilename(workspaceMatch);
    }

    return nodeRequire.resolve(request);
  };
  localRequire.cache = require.cache;
  localRequire.main = require.main;

  runtimeModule.filename = virtualFilename;
  runtimeModule.paths = Module._nodeModulePaths(nodePath.dirname(virtualFilename));
  runtimeModule.require = localRequire;
  moduleCache.set(normalizedPath, runtimeModule);
  ...
  const transformed = transform(contents, {
    ...
    production: true,
    transforms: getSucraseTransforms(normalizedPath),
  });

  runtimeModule._compile(transformed.code, virtualFilename);

  return runtimeModule.exports;
}

function compileWorkspaceWithSucrase(files, entryPath) {
  const startedAt = Date.now();
  const loadedModule = loadWorkspaceModuleWithSucrase(files, entryPath, new Map());
...
async function compileWorkspaceForRequest(files, entryPath, compiler) {
  if (compiler === "sucrase") {
    return compileWorkspaceWithSucrase(files, entryPath);
  }

  if (compiler === "rolldown") {
    return compileWorkspaceWithRolldown(files, entryPath);
  }

  return compileWorkspaceWithEsbuild(files, entryPath);
}
...
async function runServerlessExpressRequestRuntime(files, request, compilerInput) {
  const startedAt = Date.now();
  const logs = [];
  const compiler = normalizeCompilerKind(compilerInput);

  try {
    const fileMap = sanitizeWorkspaceFiles(files);
    const entryPath = extractEntryPoint(fileMap);
    const compileResult = await compileWorkspaceForRequest(
      fileMap,
      entryPath,
      compiler,
    );
      ...
      console[level] = (...args) => {
        logs.push(
          createLog(
            ...
            args.map(serializeLogValue).join(" "),
          ),
        );
    ...
    try {
      const loadedModule = compileResult.loadedModule;
      const exportedApp =
    ...
    return {
      ...
      diagnostics: [
        ...
        createDiagnostic(
          "info",
          `Stateless Express ${compiler} compile completed in ${compileResult.compileMs}ms.`,
        ),
        createDiagnostic(
          "info",
          `Stateless Express ${compiler} request completed in ${durationMs}ms.`,
        ),
      ],
      logs,
      response,
