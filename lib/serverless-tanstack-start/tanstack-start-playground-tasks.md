...
4. **Full Start Router/SSR Host:**
   - The public Start request host now renders real workspace routers and loaders, streams worker responses, dispatches server-route handlers, strips server route options from client bundles, and emits matched-route client chunks through a generated manifest. Complete route-level server/CSS chunking, redirect-aware and Request-object route proxying, and browser hydration/navigation tests.
