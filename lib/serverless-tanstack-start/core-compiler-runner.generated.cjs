...
async function compileWithStartCore(files) {
  ...
  const root = import_node_path.default.join(process.cwd(), ".tmp", "tanstack-start-core-virtual");
  const transform = await (0, import_server_functions_transform.transformStartServerFunctions)(fileMap, { root });
  return {
