...
function readPrerenderPlan(files, spaPolicy) {
  const source = files.get(importProtectionConfigFile);
  ...
  if (globalOptions.autoStaticPathsDiscovery !== void 0 && globalOptions.autoStaticPathsDiscovery !== false) {
    throw new Error(
...
function createClientOutputPreloadCollector({
  ...
  function collectPreloads(outputName, seen = /* @__PURE__ */ new Set()) {
    ...
    return [
      ...
      ...emittedStaticImports.flatMap(
        (name) => collectPreloads(name, seen)
      ),
