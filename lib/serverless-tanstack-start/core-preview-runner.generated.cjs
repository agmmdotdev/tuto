...
const picomatch = require2("picomatch");
...
const environmentFileNameSet = new Set(environmentFileNames);
...
function sanitizeWorkspaceFiles(files) {
  ...
  for (const file of files) {
    ...
    if (!normalizedPath || normalizedPath.includes("..") || normalizedPath.startsWith(".") && !environmentFileNameSet.has(normalizedPath) || path.isAbsolute(normalizedPath)) {
...
function parseEnvironmentFile(source, environment) {
...
function serverEnvironmentDefine(environment, overrides = {}) {
...
function importMetaEnvironmentDefine(environment, ssr, overrides = {}) {
  return JSON.stringify({ ...environment.client, SSR: ssr, ...overrides });
}
function importProtectionObject(value, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(`[import-protection] ${label} must be an object.`);
  }
  return value;
}
function importProtectionStringArray(value, fallback, label) {
...
function readImportProtectionPolicy(files) {
  const defaults = () => ({
    enabled: true,
    behavior: "error",
    mockAccess: "error",
  ...
  if (source === void 0) return defaults();
  let parsed;
  try {
    parsed = JSON.parse(source);
  } catch (error) {
    throw new Error(
      `[import-protection] ${importProtectionConfigFile} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  ...
  if (rawOptions === void 0) return defaults();
  const options = importProtectionObject(
    rawOptions,
    `${importProtectionConfigFile} importProtection`
  );
  const policy = defaults();
  if (options.enabled !== void 0 && typeof options.enabled !== "boolean") {
  ...
  } else if (behavior !== void 0) {
    const modes = importProtectionObject(behavior, "behavior");
    const selected = mode === "development" ? modes.dev : modes.build;
    if (selected !== void 0 && selected !== "error" && selected !== "mock") {
      throw new Error(
        `[import-protection] behavior.${mode === "development" ? "dev" : "build"} must be "error" or "mock".`
      );
    }
    policy.behavior = selected ?? (mode === "development" ? "mock" : "error");
  } else if (mode === "development") {
    policy.behavior = "mock";
  }
  if (options.mockAccess !== void 0 && options.mockAccess !== "error" && options.mockAccess !== "warn" && options.mockAccess !== "off") {
    throw new Error(
      '[import-protection] mockAccess must be "error", "warn", or "off".'
    );
  }
  policy.mockAccess = options.mockAccess ?? "error";
  if (options.log !== void 0 && options.log !== "once" && options.log !== "always") {
    throw new Error('[import-protection] log must be "once" or "always".');
  }
  policy.log = options.log ?? "once";
  ...
  for (const environment of ["client", "server"]) {
    ...
    policy[environment] = {
      specifiers: environment === "client" ? [.../* @__PURE__ */ new Set([...defaultRules.specifiers, ...specifiers])] : specifiers,
...
function readSpaPolicy(files) {
  ...
  try {
    ...
    throw new Error(
      `[spa] ${importProtectionConfigFile} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  const config = importProtectionObject(parsed, importProtectionConfigFile);
  if (config.spa === void 0) {
    return { enabled: false, maskPath: "/", outputPath: "/_shell" };
  }
  const options = importProtectionObject(
    config.spa,
  ...
  if (options.maskPath !== void 0 && typeof options.maskPath !== "string") {
    throw new Error("[spa] maskPath must be a string.");
  }
  const prerenderOptions = options.prerender === void 0 ? {} : importProtectionObject(
    options.prerender,
    `${importProtectionConfigFile} spa.prerender`
  );
  if (prerenderOptions.outputPath !== void 0 && typeof prerenderOptions.outputPath !== "string") {
    throw new Error("[spa] prerender.outputPath must be a string.");
  }
  const maskPath = options.maskPath ?? "/";
  if (!maskPath.startsWith("/") || maskPath.startsWith("//") || maskPath.length > 2048) {
    throw new Error(
  ...
  return {
    ...
    outputPath: prerenderOptions.outputPath ?? "/_shell"
  };
}
function prerenderConfigInteger(value, fallback, label, maximum) {
  if (value === void 0) return fallback;
...
function prerenderConfigBoolean(value, fallback, label) {
  if (value === void 0) return fallback;
...
function normalizePrerenderPath(value, label) {
...
function normalizePrerenderOutputPath(value, label) {
  if (typeof value !== "string" || value.length > 2048) {
    throw new Error(`[prerender] ${label} must be a path string.`);
  }
  let url;
  ...
  if (url.origin !== "http://localhost" || url.search || url.hash || segments.includes("..")) {
    throw new Error(
...
function prerenderHeaders(value, label) {
  if (value === void 0) return {};
  const headers = importProtectionObject(value, label);
  if (Object.keys(headers).length > 32 || Object.entries(headers).some(
    ([name, headerValue]) => !/^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(name) || typeof headerValue !== "string" || headerValue.length > 4096
  )) {
    throw new Error(
...
function prerenderFilterPatterns(value, label) {
  if (value === void 0) return [];
  if (!Array.isArray(value) || value.length > 32 || value.some(
  ...
  for (const pattern of value) {
    try {
      ...
      throw new Error(
        `[prerender] ${label} contains an invalid glob: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  return value;
}
function readPrerenderFilter(globalOptions) {
  if (globalOptions.filter === void 0) return void 0;
...
function prerenderPathMatchesFilter(routePath, filter) {
  if (!filter) return true;
  const pathname = new URL(routePath, "http://localhost").pathname;
  const included = filter.include.length === 0 || filter.include.some(
    (pattern) => picomatch(pattern, { dot: true })(pathname)
  );
  return included && !filter.exclude.some(
    (pattern) => picomatch(pattern, { dot: true })(pathname)
  );
}
function discoveredPrerenderPath(routeId) {
  if (routeId === "/") return "/";
  ...
  const pathSegments = segments.filter(
    (segment) => !segment.startsWith("_") && !(segment.startsWith("(") && segment.endsWith(")"))
  ).map((segment) => segment.replace(/_$/, "")).filter(Boolean);
...
function discoverPrerenderPaths(discovery) {
  return Object.entries(discovery.clientRouteIds).filter(
    ([workspacePath]) => [...discovery.clientRouteSplits.keys()].some((splitId) => {
      if (!splitId.startsWith(`${workspacePath}?tsr-split=`)) return false;
      return splitId.slice(splitId.indexOf("=") + 1).split("---").includes("component");
    })
  ).map(([, routeId]) => discoveredPrerenderPath(routeId)).filter((routePath) => routePath !== null).sort((left, right) => left.localeCompare(right));
}
function readPrerenderPlan(files, spaPolicy, discovery) {
  const source = files.get(importProtectionConfigFile);
  ...
  try {
    ...
    throw new Error(
      `[prerender] ${importProtectionConfigFile} is not valid JSON: ${error instanceof Error ? error.message : String(error)}`
    );
  }
  const config = importProtectionObject(parsed, importProtectionConfigFile);
  const globalOptions = config.prerender === void 0 ? {} : importProtectionObject(
    config.prerender,
    `${importProtectionConfigFile} prerender`
  );
  if (globalOptions.enabled !== void 0 && typeof globalOptions.enabled !== "boolean") {
    throw new Error("[prerender] enabled must be a boolean.");
  }
  if (globalOptions.autoStaticPathsDiscovery !== void 0 && typeof globalOptions.autoStaticPathsDiscovery !== "boolean") {
  ...
  const pageEntries = rawPages.map((rawPage, index) => {
    if (typeof rawPage === "string") {
      return { path: rawPage, prerender: {} };
    }
    const page = importProtectionObject(rawPage, `pages[${index}]`);
    const pageOptions = page.prerender === void 0 ? {} : importProtectionObject(page.prerender, `pages[${index}].prerender`);
    if (pageOptions.enabled !== void 0 && typeof pageOptions.enabled !== "boolean") {
      throw new Error(
  ...
  const enabled = spaPolicy.enabled || globalOptions.enabled !== false && (globalOptions.enabled === true || enabledByPage);
  if (!enabled) return void 0;
  const filter = readPrerenderFilter(globalOptions);
  const resolvePage = (rawPath, pageOptions, label) => {
  ...
  const explicitPages = pageEntries.map(
    ({ path: pagePath, prerender }, index) => resolvePage(pagePath, prerender, `pages[${index}].prerender`)
  ).filter((page) => page !== null);
  const pagesByPath = /* @__PURE__ */ new Map();
  const automaticDiscoveryEnabled = globalOptions.autoStaticPathsDiscovery === true || globalOptions.autoStaticPathsDiscovery !== false && (globalOptions.enabled === true || enabledByPage);
...
const importProtectionGlobCache = /* @__PURE__ */ new Map();
function matchesImportProtectionPattern(value, pattern) {
...
function firstImportProtectionMatch(value, patterns) {
  return patterns.find(
    (pattern) => matchesImportProtectionPattern(value, pattern)
  );
}
function importProtectionWorkspacePath(filePath) {
  const cleanPath = filePath.split(/[?#]/, 1)[0]?.replaceAll("\\", "/") ?? "";
  return fromVirtualWorkspacePath(cleanPath) ?? cleanPath.replace(/^\/+/, "");
}
function shouldCheckImportProtectionImporter(importer, policy) {
  const relativePath = importProtectionWorkspacePath(importer);
  if (firstImportProtectionMatch(relativePath, policy.exclude) || firstImportProtectionMatch(relativePath, policy.ignoreImporters)) {
    return false;
  }
...
function workspaceMarkerRestrictions(files) {
  const serverOnly = /* @__PURE__ */ new Set();
  const clientOnly = /* @__PURE__ */ new Set();
...
function createImportProtectionPlugin(files, environment, root) {
  const run = activeImportProtectionRun;
  const policy = run?.policy ?? readImportProtectionPolicy(files);
  const restrictions = policy.enabled ? workspaceMarkerRestrictions(files) : { clientOnly: /* @__PURE__ */ new Set(), serverOnly: /* @__PURE__ */ new Set() };
  ...
  const mockExportNames = (importer, specifier) => {
  ...
  const mockModule = (names, message) => {
    const namedExports = names.filter((name) => name !== "default").map((name) => `export { mock as ${name} };`).join("\n");
    const report = policy.mockAccess === "off" ? "" : `if (!reported) { reported = true; console.${policy.mockAccess}(new Error(message)); }`;
    return `
    ...
    setup(buildApi) {
      buildApi.onResolve({ filter: /.*/ }, (args) => {
        if (args.path === serverOnlyMarker || args.path === clientOnlyMarker) {
          ...
          const denied = environment === "client" && args.path === serverOnlyMarker || environment === "server" && args.path === clientOnlyMarker;
          if (denied) {
            return deny({
        ...
        const resolved = (rootedModule && files.has(rootedModule) ? rootedModule : null) ?? (files.has(args.path) ? args.path : null) ?? (args.importer ? resolveWorkspaceImport(files, args.path, args.importer) : null);
        ...
        const deniedByMarker = environment === "client" ? restrictions.serverOnly.has(cleanResolved) : restrictions.clientOnly.has(cleanResolved);
        if (deniedByFile || deniedByMarker) {
          return deny({
            importer: args.importer,
            specifier: args.path,
            resolved: cleanResolved,
            reason: deniedByFile ? `Denied by file pattern: ${deniedByFile}` : `Denied by ${environment === "client" ? "server-only" : "client-only"} marker`
          });
        }
        return null;
      });
...
function findWorkspaceScriptFile(files, candidatePath) {
  const match = findWorkspaceFile(files, candidatePath);
  return match && /\.[cm]?[jt]sx?$/.test(match) ? match : null;
}
const workspacePathAliasCache = /* @__PURE__ */ new WeakMap();
function readWorkspacePathAliases(files) {
  ...
  const baseUrl = typeof options.baseUrl === "string" ? normalizeWorkspacePath(options.baseUrl).replace(/^\.\/?/, "") : "";
  ...
  const aliases = Object.entries(paths).flatMap(([pattern, targets]) => {
    ...
    (left, right) => right.patternPrefix.length + right.patternSuffix.length - (left.patternPrefix.length + left.patternSuffix.length)
  );
  const result = { aliases, baseUrl };
  workspacePathAliasCache.set(files, result);
  return result;
}
function resolveWorkspacePathAlias(files, source) {
  const { aliases, baseUrl } = readWorkspacePathAliases(files);
  for (const alias of aliases) {
    if (!source.startsWith(alias.patternPrefix) || !source.endsWith(alias.patternSuffix)) {
      continue;
    }
    ...
    for (const target of alias.targets) {
      const candidate = target.includes("*") ? target.replaceAll("*", wildcardValue) : wildcardValue ? null : target;
...
function buildSsrPreviewRedirect(revision, rpcToken, spaPolicy) {
...
async function buildNativeServerBundle({
  ...
  if (Object.keys(transform.serverFnsById).length === 0 && !routerModule && !customServerModule) {
...
globalThis.${import_kernel_manifest_generated.default.server.handlerKey} = (request, requestOptions = {}) =>
  ...
  const result = await (0, import_esbuild.build)({
    ...
    define: {
      ...
      "process.env": serverEnvironmentDefine(environment, {
        TSS_SERVER_FN_BASE: import_kernel_manifest_generated.default.server.serverFnBase
      })
    },
...
` : "";
  ...
  if (pathname === ${JSON.stringify(import_kernel_manifest_generated.default.rsc.actionInternalPath)}) {
  ...
  for (const [outputPath, output] of Object.entries(metafile.outputs)) {
    ...
    const routePath = Object.keys(routeIds).find(
      (workspacePath) => Object.keys(output.inputs).some(
        (inputPath) => inputPath.includes(`${workspacePath}?tsr-split=`) && // Hydrate virtual modules are dynamic child entries, not route entry
...
function buildRscClientReferenceDeps({
  ...
  for (const [outputPath, output] of Object.entries(metafile.outputs)) {
    ...
    deps[reference] = {
      css: output.cssBundle ? [routeStyleUrl(styleAssetBase, relativeOutputName(output.cssBundle))] : [],
...
${customClientModule ? `await import(${JSON.stringify(`./${customClientModule}`)});` : `startTransition(() => {
...
${serverBuild.code}` : serverBuild.code;
    ...
    return {
      buildMetrics,
      success: true,
      html: ssrClientBuild.code ? buildSsrPreviewRedirect(revision, rpcToken, spaPolicy) : injectPreviewAssets({
        html,
