...
function isClientOnlyFile(filePath) {
  ...
  return {
    ...
    setup(buildApi) {
      buildApi.onResolve({ filter: /.*/ }, (args) => {
        ...
        if (environment === "client" && args.path === "@tanstack/react-start/server") {
          throw importProtectionError({
            environment,
            importer: args.importer,
            specifier: args.path,
        ...
        const deniedByFile = environment === "client" ? isServerOnlyFile(cleanResolved) : isClientOnlyFile(cleanResolved);
        ...
        if (deniedByFile || deniedByMarker) {
          throw importProtectionError({
            ...
            reason: deniedByFile ? `Denied by file pattern: **/*.${environment === "client" ? "server" : "client"}.*` : `Denied by ${environment === "client" ? "server-only" : "client-only"} marker`
          });
        }
        return null;
      });
