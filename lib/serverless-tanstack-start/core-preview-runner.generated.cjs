...
async function loadTailwindStylesheet(files, id, base) {
  const workspaceFilePath = id.startsWith(".") || id.startsWith("/") ? (() => {
    ...
    return workspaceBasePath ? findWorkspaceFile(files, path.normalize(path.join(workspaceBasePath, id))) : null;
...
async function loadTailwindModule(id, _base, resourceHint) {
  ...
  const resolvedPath = require2.resolve(id, { paths: [absoluteWorkingDirectory] });
...
function resolveWorkspaceModule(files, args) {
...
function createWorkspacePlugin(files, originalFiles) {
  return {
    ...
    setup(buildApi) {
      buildApi.onResolve(
        { filter: /.*/ },
        (args) => resolveWorkspaceModule(files, args)
      );
      ...
      buildApi.onLoad({ filter: /.*/, namespace: "tanstack-start-core-shim" }, (args) => ({
        contents: args.path === "@tanstack/react-start/client-rpc" ? createClientRpcShimSource(
...
function buildFailurePreview(diagnostics) {
  const body = diagnostics.map((diagnostic) => `<article><strong>${escapeHtml(
    diagnostic.filePath ?? "build"
  )}</strong><pre>${escapeHtml(diagnostic.message)}</pre></article>`).join("");
...
function normalizeBuildError(error) {
  if (isEsbuildError(error) && Array.isArray(error.errors) && error.errors.length > 0) {
    return error.errors.map(
      (entry) => createDiagnostic("error", entry.text || entry.message || "TanStack core preview failed.", {
...
async function compilePreview(files) {
  ...
  try {
    ...
    const transform = await (0, import_server_functions_transform.transformStartServerFunctions)(originalFileMap, {
      root: import_node_path.default.join(absoluteWorkingDirectory, ".tmp", "tanstack-start-core")
    });
