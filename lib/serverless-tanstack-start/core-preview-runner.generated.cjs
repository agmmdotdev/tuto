...
function createTailwindScanInputs(files) {
  return [...files.entries()].map(([filePath, content]) => {
    ...
    return tailwindSourceExtensions.has(extension) && content ? { file: toVirtualWorkspacePath(filePath), content, extension } : null;
  }).filter(Boolean);
}
...
async function compileTailwindCss(files, filePath, contents) {
  ...
  let candidates = [];
  if (compiledCss.root !== "none" && (compiledCss.features & tailwindcss.Features.Utilities) !== 0) {
