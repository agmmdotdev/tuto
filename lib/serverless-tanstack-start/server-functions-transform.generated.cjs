...
var defaultRouteSplitGroupings = [
  ["component"],
  ["errorComponent"],
  ["notFoundComponent"]
];
async function importStartCompilerInternals() {
  ...
  const hydrate = await import((0, import_node_url.pathToFileURL)(
    import_node_path.default.join(packageRoot, "dist", "esm", "hydrate-when-transform.js")
  ).toString());
  return {
...
function routeIdFromCode(code) {
  return code.match(/\bcreateFileRoute\s*\(\s*(["'`])([^"'`]+)\1\s*\)/)?.[2] ?? (/\bcreateRootRoute\s*\(/.test(code) ? "__root__" : void 0);
}
function routeSplitModuleId(workspacePath, grouping) {
  return `${workspacePath}?tsr-split=${grouping.slice().sort().join("---")}`;
}
async function compileRoutes(files, root, options = {}) {
  const {
  ...
  const clientRouteSplits = /* @__PURE__ */ new Map();
  for (const [workspacePath, code] of [...files]) {
    ...
    const codeSplitGroupings = detectCodeSplitGroupingsFromRoute({ code, filename: id }).groupings ?? defaultRouteSplitGroupings;
    ...
    const result = compileCodeSplitReferenceRoute({
      ...
      codeSplitGroupings,
      compilerPlugins: [],
      deleteNodes: options.deleteNodes ?? /* @__PURE__ */ new Set(),
      filename: id,
      id,
    ...
    files.set(
      workspacePath,
      options.stripRouteCssImports ? result.code.replace(
        /\bimport\s+(["'])[^"'\r\n]+\.css(?:\?[^"'\r\n]*)?\1\s*;?/g,
        ""
      ) : result.code
    );
...
async function transformStartServerFunctions(fileMap, options = {}) {
  const { createHydrateCompilerPlugin, createStartCompiler, detectKindsInCode } = await importStartCompilerInternals();
  ...
  for (const [workspacePath, code] of fileMap.entries()) {
    ...
    if (!code.includes("createServerFn") && !code.includes("createMiddleware") && !code.includes("createServerOnlyFn") && !code.includes("createClientOnlyFn") && !code.includes("createIsomorphicFn") && !code.includes("Hydrate")) {
      clientFiles.set(workspacePath, code);
      serverFiles.set(workspacePath, code);
      continue;
    }
  ...
  const seenHydrateModules = /* @__PURE__ */ new Set();
