...
const staticServerFunctionCollectorKey = "__TUTO_TANSTACK_START_STATIC_SERVER_FUNCTION_COLLECTOR__";
...
async function cleanup() {
  ...
  delete globalThis[staticServerFunctionCollectorKey];
  delete globalThis[import_kernel_manifest_generated.default.rsc.globalKey];
  delete globalThis[import_kernel_manifest_generated.default.rsc.actionEncryptionKeyGlobalKey];
...
async function initialize(command) {
  ...
  if (actionEncryptionKeyBytes.byteLength !== 32 || actionEncryptionKeyBytes.toString("base64") !== command.rscActionEncryptionKey) {
    throw new Error("Invalid TanStack Start RSC action encryption key.");
  }
  ...
  globalThis[import_kernel_manifest_generated.default.rsc.actionEncryptionKeyGlobalKey] = command.rscActionEncryptionKey;
  ...
  handler = (request, options) => {
    ...
    if ((pathname === import_kernel_manifest_generated.default.rsc.internalPath || pathname === import_kernel_manifest_generated.default.rsc.actionInternalPath) && typeof rscHandler === "function") {
...
async function execute(command) {
  ...
  const staticServerFunctionCache = {};
  let staticServerFunctionBytes = 0;
  globalThis[staticServerFunctionCollectorKey] = ({
    body,
    url
  }) => {
    ...
    if (existing !== void 0 && existing !== body) {
      throw new Error(
    ...
    if (bytes > maxResponseBytes || staticServerFunctionBytes + bytes > maxResponseBytes) {
  ...
  const result = {
    ...
    ...Object.keys(staticServerFunctionCache).length > 0 ? { staticServerFunctionCache } : {},
    ...responseHead
  };
