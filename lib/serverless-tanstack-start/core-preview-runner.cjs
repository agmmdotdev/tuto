...
async function importStartCompilerInternals() {
  ...
  const esmRoot = nodePath.join(packageRoot, "dist", "esm", "start-compiler");
  const host = await import(pathToFileURL(nodePath.join(esmRoot, "host.js")).toString());
  const compiler = await import(pathToFileURL(nodePath.join(esmRoot, "compiler.js")).toString());
  const config = await import(pathToFileURL(nodePath.join(esmRoot, "config.js")).toString());
...
function toAbsoluteModuleId(root, workspacePath) {
  return nodePath.join(root, ...workspacePath.split("/"));
}

function toWorkspacePath(root, absoluteId) {
  const cleanId = absoluteId.split("?")[0];
  const relativePath = nodePath.relative(root, cleanId).replaceAll("\\", "/");
...
function createCoreResolver(root, fileMap) {
  ...
  return async function resolveId(source, importer) {
    ...
    const basePath = nodePath.resolve(nodePath.dirname(importerPath), source);
...
async function transformClientWithStartCore(fileMap) {
  const {
    createStartCompiler,
    detectKindsInCode,
  ...
  const root = nodePath.join(absoluteWorkingDirectory, ".tmp", "tanstack-start-core");
  const serverFnsById = {};
  ...
  const transformed = new Map(fileMap);
  ...
  for (const [workspacePath, code] of fileMap.entries()) {
    ...
    if (result?.code) transformed.set(workspacePath, result.code);
  }

  return { transformed, serverFnsById };
}
...
async function compilePreview(files) {
  ...
  try {
    ...
    const { transformed, serverFnsById } = await transformClientWithStartCore(originalFileMap);
