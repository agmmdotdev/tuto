...
function isRedirectControlValue(value) {
  return typeof Response !== "undefined" && value instanceof Response && typeof value.options === "object" && value.options !== null;
}
function isNotFoundControlValue(value) {
  return typeof value === "object" && value !== null && value.isNotFound === true;
}
function serializeHeaders(headers) {
  return Object.fromEntries(new Headers(headers).entries());
}
function serializeRedirectOptions(options) {
  const serialized = {};
  for (const [key, value] of Object.entries(options)) {
    if (typeof value === "undefined" || key === "headers" || key === "throw" || key.startsWith("_")) {
      continue;
    }
...
function serializeRpcControl(value) {
  if (isRedirectControlValue(value)) {
    return {
      type: "redirect",
      href: typeof value.options.href === "string" ? value.options.href : value.headers.get("location") ?? void 0,
...
function normalizeError(error) {
  ...
  if (control) {
    const message2 = control.type === "redirect" ? `Redirect to ${control.href ?? control.headers.location ?? "unknown location"}` : "Not found";
    return {
      success: false,
      control,
      error: message2,
      diagnostics: [createDiagnostic("info", message2)]
    };
  }
