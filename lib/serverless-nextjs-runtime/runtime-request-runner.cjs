...
function sanitizeWorkspaceKey(workspaceKey) {
...
async function resetWorkspaceFiles(workspaceDirectory) {
  ...
  await Promise.all(
    entries.map(async (entry) => {
      ...
      await fs.rm(path.join(workspaceDirectory, entry.name), {
        recursive: true,
        force: true,
      });
    }),
...
async function createWorkspace(fileMap, logs, workspaceKey) {
  await pruneOldWorkspaces();
  const workspaceDirectory = path.join(
    workspaceRoot,
    sanitizeWorkspaceKey(workspaceKey),
  );

  await fs.mkdir(workspaceDirectory, { recursive: true });
  await resetWorkspaceFiles(workspaceDirectory);
  const hasDependencyTree = await fs
    .stat(path.join(workspaceDirectory, "node_modules"))
    .then(() => true)
    .catch(() => false);
  ...
  if (!hasDependencyTree) {
    ...
    pushLog(logs, "info", "Reusing warm temp workspace dependencies.");
  }
